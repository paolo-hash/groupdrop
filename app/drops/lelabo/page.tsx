...
/* ─────────────────────────────────────────────────────────────
   Page
   CHANGE: Only the component name and Supabase slug changed
   from AesopPage → LeLaboPage and "aesop" → "lelabo".
   Everything else is structurally identical to aesop-page.tsx
   so both pages share the exact same look and feel.
───────────────────────────────────────────────────────────── */
export default function LeLaboPage() {
  ...
  /*
    CHANGE: Added `mounted` state to trigger the progress-bar entrance
    animation after hydration — mirrors the homepage + Aesop pattern.
  */
  ...
  async function fetchData() {
    ...
    /*
      CHANGE: slug updated from "aesop" → "lelabo" — the only data-layer
      difference between this page and the Aesop page.
    */
  ...
  /*
    CHANGE: 100ms mounted flag for progress bar animation — identical to
    homepage and Aesop pattern.
  */
  useEffect(() => {
  ...
  /*
    CHANGE: Loading screen uses cream background + tracked uppercase label —
    identical treatment to Aesop page, replacing the plain neutral-50 spinner.
  */
  if (loading) {
    return (
      <>
  ...
  /* ── Render ───────────────────────────────────────────────── */
  /*
    CHANGE: The entire render below is byte-for-byte identical to aesop-page.tsx
    (minus the component name). This is intentional — both drop pages share the
    same layout, token usage, and interaction patterns. The SHARED_STYLES
    constant at the bottom ensures visual consistency across all three pages
    (homepage, Aesop, Le Labo) from a single source of truth.
  */
  return (
    <>
      <style>{SHARED_STYLES}</style>
      ...
      <main style={{ minHeight: '100vh', backgroundColor: 'var(--cream)', color: 'var(--ink)' }}>
        ...
        <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 28px' }}>
          ...
          <section style={{ paddingTop: '100px' }}>
            ...
            {/* Drop name — italic Cormorant Garamond, same as Aesop */}
                ...
                {skus.map((sku) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Product image placeholder with brand initial monogram */}
                      <div style={{
                        height: '100px',
                        backgroundColor: 'var(--parchment)',
                        borderRadius: '2px',
...
/* ─────────────────────────────────────────────────────────────
   ...
   Identical across page.tsx, aesop-page.tsx, and lelabo-page.tsx.
   When adding a new drop page, copy this constant in unchanged.
───────────────────────────────────────────────────────────── */
const SHARED_STYLES = `
